...
async def main():
    ...
    model = init_chat_model(
        "openai:gpt-4o-mini", api_key=os.getenv("OPENAI_API_KEY", "")
    )
    ...
    client = MultiServerMCPClient(
        {
            "google-rss-mcp": {
                "command": "google-rss-mcp",
                "args": [],
                "transport": "stdio",
                "env": {"GOOGLE_RSS_LANGUAGE": "ko", "GOOGLE_RSS_REGION": "KR"},
            },
        }
    )
    ...
    tools_table = Table(
        title="🔧 Available Google News RSS FastMCP Tools",
        show_header=True,
    ...
    try:
        ...
        if messages and hasattr(messages[-1], "content") and messages[-1].content:
            ...
            console.print(
                Panel("[bold red]❌ No response found[/bold red]", border_style="red")
            )
    ...
    # Footer
    console.print(
        Panel("[dim]✨ All tests completed successfully![/dim]", border_style="dim")
    )
